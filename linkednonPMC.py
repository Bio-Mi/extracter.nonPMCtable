...
base_dir= '/home/moibrahim/Desktop/link/'
...
for i,k in enumerate(final_new_menu):
    ...
    for l, j in enumerate(files):  
        ...
        for index, obj in enumerate(thelist):
            for e in obj.find_all(attrs={'style':['display:none','visibility:hidden']}):
                e.extract()

            for ref in obj.find_all(class_=['supplementary-material','figpopup','popnode','bibr']):
                ref.extract()
            
            process_supsub(obj)
            process_em(obj)      

            for table_num, table in enumerate(obj.find_all('table',recursive=True)): 
