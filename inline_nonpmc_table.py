...
def table_to_2d(t):
    ...
    for row_idx, row in enumerate(rows):
        ...
        for col_idx, cell in enumerate(row.findAll(['td', 'th'])):
            ...
            # Clean the cell
            #To remove all potential whitespace
            value = value.strip().replace('\u2005',' ').replace('\u2010','').replace('\u2009',' ').replace('   ','').replace('\xa0',' ').replace('         ','').replace('\n','').replace('            ','')
            
            if value.startswith('(') and value.endswith(')'):
                value = value[1:-1]
            #To alter x 10 to e
            if re.match('.*((\d+.\d+)|(\d+))[\s{0,1}][××][\s{0,1}]10([_–−-]{0,1})(.?\d+)',value):
                value = value.replace(' × 10_','e').replace('×10_','e').replace('× 10','e').replace('−','-').replace(' ','').replace('–','-')   
            
            #To change log 10^-3 to log 10E-3
            if re.search('log\s?\d+_\s?\d+',value):
                l= re.search('(.*log\s?\d+)_\s?(\d+.*)',value)
                value = l.group(1)+ str('E') + l.group(2) 
...
for ind , nam in enumerate(folders):
    file_list = get_files(nam)
    file_name= re.findall(r'(\d+)_DOI',str(file_list))
    thelist=[filepath(file_name) for file_name in file_list] 

    res={}
    for key in thelist: 
        for value in file_name: 
            res[key] = value 
            file_name.remove(value)
            break
    ...
    for index, obj in enumerate(thelist):
        ...
        for table_num, table in enumerate(soup.find_all('table',recursive=True)): 
                ...
                if re.match('.*/American_journal_of_medical_genetics',nam):

                    if  table.find_previous('header','article-table-caption'):
                        caption = table.find_previous('header','article-table-caption').get_text().strip().replace('\u2005',' ').replace('\u2010','').replace('\u2009',' ').replace('   ','').replace('\xa0',' ').replace('         ','').replace('\n','').replace('            ','')
                    else:
                        caption = ' '#table.find('caption').get_text()

                    if table.parent.find_next_siblings('div','article-section__table-footnotes'):
                        footer=  [i.get_text().strip().replace('\u2005',' ').replace('\u2010','').replace('\u2009',' ').replace('   ','').replace('\xa0',' ').replace('         ','').replace('\n','').replace('            ','') for i in table.parent.find_next_siblings('div','article-section__table-footnotes')]
                    else:
                        footer = ' '

                elif re.match('.*/Blood',nam):
                    ...
                    if  table.find_next('div','table-wrap-foot'):
                        footer=  [i.get_text().strip().replace('\u2005',' ').replace('\u2010','').replace('\u2009',' ').replace('   ','').replace('\xa0',' ').replace('         ','').replace('\n','').replace('            ','')  for i in  table.find_next('div','table-wrap-foot')]
                    else:
                        footer = ' '
                    

                elif re.match('.*/American_journal_of_respiratory_and_critical_care_medicine',nam):
                    if  table.caption:
                ...
                elif re.match('.*/American_journal_of_human_genetics',nam):
                    
                    if  table.find_previous('div','inline-table__head'):
                        caption = table.find_previous('div','inline-table__head').get_text().strip().replace('\u2005',' ').replace('\u2010','').replace('\u2009',' ').replace('   ','').replace('\xa0',' ').replace('         ','').replace('\n','').replace('            ','')
                    else:
                        caption = ' '#table.find('caption').get_text()

                    if table.find_next('div','inline-table__tail'):
                        footer= [i.get_text().strip().replace('\u2005',' ').replace('\u2010','').replace('\u2009',' ').replace('   ','').replace('\xa0',' ').replace('         ','').replace('\n','').replace('            ','')for i in table.find_next('div','inline-table__tail')]
                    else:
                        footer = ' '
                ...
                if  superrow_idx==[]:
                    ...
                    for table in cur_table:
                        ...
                        #''''''''''changeing area''''''''''''for col_idx in range(len(headers)[0])'''''''''''''''''''''
                        new_header = []
                        if header_idx:
                            if range(len(headers)):
